...
# Load data
loc_df = pd.read_csv('groundtruth.csv', header=None, names=['x', 'y'])
toa_df = pd.read_csv('filtered_distance.csv', header=None, names=['toa1', 'toa2', 'toa3', 'toa4'])
...
# HW 데이터로 테스트
test_toa_df = pd.read_csv('TOA.csv', header=None, names=['toa1', 'toa2', 'toa3', 'toa4'])
test_toa_df = test_toa_df.apply(pd.to_numeric, errors='coerce').dropna().reset_index(drop=True)
test_toa_df = test_toa_df.iloc[:250]  # [:250] 한바퀴 해당
test_toa_df = test_toa_df / 3e8

# HW 예측좌표
actual_loc_df = pd.read_csv('Loc.csv', header=None, names=['x', 'y'])
actual_loc_df = actual_loc_df.apply(pd.to_numeric, errors='coerce').dropna().reset_index(drop=True)
actual_loc_df = actual_loc_df.iloc[:250]  # [:250] 한바퀴 해당
...
test_x_seq = np.array([test_x_data[i:i+seq_length] for i in range(len(test_x_data) - seq_length)])
...
plt.scatter(actual_loc_df['x'], actual_loc_df['y'], c='green', label='Actual')

plt.xlabel('X coordinate')
plt.ylabel('Y coordinate')
plt.legend()
plt.title('Predicted Coordinates, Actual Coordinates and Distance MSE to Rectangle')
plt.show()